...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (1121, 594897841))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.97.0'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-66/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], '2a65f2917d2ed86a273d1fb2e45f25de232e3563')
        self.assertEqual(release['sourceRevision'], '1518e15e248b59c6470cc6931023eaf6f1ac363c')
        self.assertEqual(release['sourceTree'], 'c618b5d01fd0742a5dfc8c55c9d851d1d21d58e8')
        self.assertEqual(release['sourceQualification'], {
            'bytes': 14164,
            'sha256': 'dcea76d94239d0aab2306979cd4f98346074a04c0ee7fec2a5d24dfc8a05ca7c',
        ...
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.97.0/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (1118, 594896447))
